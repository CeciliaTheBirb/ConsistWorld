...
from wan.utils.accel import device_type as _default_device_type
...
def compute_relative_poses(
    ...
    trans_scale: float = None,
) -> torch.Tensor:
    # ``ref_pose`` (a single [4,4] c2w) makes poses relative to a shared
    # world anchor instead of this view's own frame 0 — required for multi-view so
    # the inter-view camera placement survives (framewise deltas are anchor-
    # independent and would erase it). With a shared anchor, frame 0 is NOT forced
    # to identity (a non-source view's frame 0 carries its offset from the anchor),
    # and normalization uses the caller-supplied shared ``trans_scale`` so source
    # and target keep a consistent metric scale.
    if ref_pose is not None:
        ref_w2cs = SE3_inverse(ref_pose.reshape(1, 4, 4))
    ...
    if trans_scale is not None:
        if trans_scale > 0:
            relative_poses[:, :3, 3] = relative_poses[:, :3, 3] / trans_scale
    elif normalize_trans: # note refer to camctrl2: "we scale the coordinate inputs to roughly 1 standard deviation to simplify model learning."
